...
async function initializeAdminDashboard() {
  ...
  if (!bridge) {
    window.addEventListener('growing-seed-firebase-ready', initializeAdminDashboard, { once: true });
    return;
  }

  const accessGate = el('adminAccessGate');
  const accessMessage = el('adminAccessMessage');
  try {
    const session = await bridge.ready;
    if (!session) {
      accessMessage.textContent = 'Sign in to the game first, then open this dashboard again.';
      accessGate.hidden = false;
      return;
    }

    const role = await bridge.getCurrentUserRole();
    if (!DASHBOARD_ROLES.includes(role)) {
      accessMessage.textContent = 'Your account does not have dashboard access.';
      accessGate.hidden = false;
      return;
    }
    ...
    console.error('Admin dashboard access check failed.', error);
    accessMessage.textContent = 'Dashboard access could not be verified. Check your Firebase connection and try again.';
    accessGate.hidden = false;
  }
}
